            ...
            if self.db_sampler:
                ...
                if sampled_dict is not None:
                    ...
                    for k in range(sampled_point_indices.shape[1]):
                        rest_mask &= ~sampled_point_indices[:, k]

                    points = np.concatenate([sampled_points, points[rest_mask]], axis=0)
...
class Voxelization(object):
    ...
    def __call__(self, res, info):
        ...
        # add by sgs: remove points outside of range
        res["lidar"]["points"] = prep.filter_points_outside_range(res["lidar"]["points"], pc_range)

        if double_flip:
            res["lidar"]["xflip_points"] = prep.filter_points_outside_range(res["lidar"]["xflip_points"], pc_range)
...
class AssignLabel(object):
    def __init__(self, **kwargs):
        ...
        self.pc_range = assigner_cfg.pc_range
        self.voxel_size = assigner_cfg.voxel_size
        self.pc_range = np.array(assigner_cfg.pc_range, dtype=np.float32)
        self.voxel_size = np.array(assigner_cfg.voxel_size, dtype=np.float32)
