...
def create_groundtruth_database(
    ...
    **kwargs,
    ...
    for index in tqdm(range(len(dataset))):
        ...
        if num_obj == 0:
            continue
        ct_gt_boxes = copy.deepcopy(gt_boxes)
        ct_gt_boxes[:, 3:6] *= extra_scale
        # point_indices = box_np_ops.points_in_rbbox(points, ct_gt_boxes)
        point_indices = box_np_ops.points_in_rbbox2d(points, ct_gt_boxes)

        # bg_mask = np.ones(len(points), dtype=np.bool)
        # fg_mask = np.zeros(len(points), dtype=np.bool)
        # for k in range(len(gt_boxes)):
        #     bg_mask &= ~point_indices[:, k]
        #     fg_mask |= point_indices[:, k]
        #
        # from tools.visual import draw_points, draw_boxes
        # viser = draw_points(points[bg_mask], point_size=0.5, color=(1, 0, 0))
        # viser = draw_points(points[fg_mask], viser, point_size=3, color=(0, 0, 1))
        # viser = draw_boxes(gt_boxes, viser, color=(0, 1, 0))
        # viser = draw_boxes(ct_gt_boxes, viser, color=(0, 0, 1))
        # viser.run()
