...
def train_detector(model, dataset, cfg, distributed=False, validate=False, logger=None, tb_logger=None):
    if logger is None:
    ...
    trainer = Trainer(
        model, batch_processor, optimizer, lr_scheduler, cfg.work_dir, cfg.log_level, logger, tb_logger
    )
