...
class ScatterBevMaxFunction(Function):
    @staticmethod
    def forward(ctx, src:torch.Tensor, index:torch.Tensor, index2bev:torch.Tensor, BHW:int):
        """
        Args:
            index2bev: (N,)
            index: (L, )
        ...
        Returns:
            out: (C, BHW)
        """
        assert index2bev.is_contiguous()
        assert index.is_contiguous()
        assert src.is_contiguous()

        C, L = src.size()
        arg = torch.full([C, BHW], -1, dtype=index.dtype, device=index.device, requires_grad=False)
        out = src.new_zeros([C, BHW])

        pillar_cuda.update_index_kernel_wrapper(index2bev, index)
        pillar_cuda.scatter_max_wrapper(index, src, arg, out)
...
scatter_bev_max = ScatterBevMaxFunction.apply
