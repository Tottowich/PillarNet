...
class BallQuery(Function):
    ...
    def forward(ctx, radius: float, nsample: int, xyz: torch.Tensor, xyz_batch_cnt: torch.Tensor,
        ...
        pointnet2.ball_query_wrapper(radius, nsample, new_xyz, new_xyz_batch_cnt, xyz, xyz_batch_cnt, idx)
...
class RoICylinderPool3dFunction(Function):
    @staticmethod
    def forward(ctx, xyz: torch.Tensor, xyz_batch_cnt: torch.Tensor,
                rois: torch.Tensor, roi_batch_cnt: torch.Tensor, nsample: int, extra: float):
        """
        Args:
            extra: float, extra proposal meters for width and height
            nsample: int, maximum number of features in the balls
            xyz: (N1 + N2 ..., 3) xyz coordinates of the features
            xyz_batch_cnt: (batch_size), [N1, N2, ...]
            rois: (M1+M2 ..., 7)  [x y z w l h ry ...]
            roi_batch_cnt: (M1, M2, ...)
        ...
        assert roi_batch_cnt.is_contiguous()

        M = rois.shape[0]
        idx = torch.full((M, nsample), -1, dtype=torch.int32, device=rois.device)

        pointnet2.roi_cylinder_pool3d_wrapper(extra, rois, roi_batch_cnt, xyz, xyz_batch_cnt, idx)
...
roi_cylinder_pool3d = RoICylinderPool3dFunction.apply


class RoICuboidPool3dFunction(Function):
    @staticmethod
    def forward(ctx, xyz: torch.Tensor, xyz_batch_cnt: torch.Tensor,
                rois: torch.Tensor, roi_batch_cnt: torch.Tensor, nsample: int, extra: float):
        """
        ...
        pointnet2.roi_point_pool3d_wrapper(extra, rois, roi_batch_cnt, xyz, xyz_batch_cnt, idx)
...
roi_cuboid_pool3d = RoICuboidPool3dFunction.apply
...
class GroupingOperation(Function):
    ...
    def forward(ctx, features: torch.Tensor, features_batch_cnt: torch.Tensor,
        ...
        output = torch.zeros((M, C, nsample), dtype=features.dtype, device=features.device)

        pointnet2.group_points_wrapper(features, features_batch_cnt, idx, idx_batch_cnt, output)
    ...
    def backward(ctx, grad_out: torch.Tensor):
        ...
        pointnet2.group_points_grad_wrapper(grad_out_data, idx, idx_batch_cnt,
                                            features_batch_cnt, grad_features.data)
...
class QueryAndGroupTusimple(nn.Module):
    def __init__(self, extra: float, nsample: int, cuboid: bool = True):
        """
        Args:
            radius: float, radius of ball
            nsample: int, maximum number of features to gather in the ball
        ...
        """
        super().__init__()
        self.extra, self.nsample, self.cuboid = extra, nsample, cuboid

    def forward(self, xyz: torch.Tensor, xyz_batch_cnt: torch.Tensor,
                rois: torch.Tensor, roi_batch_cnt: torch.Tensor,
                features: torch.Tensor = None):
        """
        Args:
            xyz: (N1 + N2 ..., 3) xyz coordinates of the features
            xyz_batch_cnt: (batch_size), [N1, N2, ...]
            rois: (M1 + M2 ..., C) [x y z w l h ry ...]
            roi_batch_cnt: (batch_size), [M1, M2, ...]
            features: (M1 + M2 ..., C) tensor of features to group
        ...
        # idx: (M1 + M2 ..., nsample), empty_ball_mask: (M1 + M2 ...)
        if self.cuboid:
            idx, empty_ball_mask = roi_cylinder_pool3d(xyz, xyz_batch_cnt, rois, roi_batch_cnt, self.extra)
        else:
            idx, empty_ball_mask = roi_cuboid_pool3d(xyz, xyz_batch_cnt, rois, roi_batch_cnt, self.extra)
        grouped_xyz = grouping_operation(xyz, xyz_batch_cnt, idx, roi_batch_cnt)  # (M1 + M2, 3, nsample)
        grouped_xyz -= rois[..., :3].unsqueeze(-1)

        if features is not None:
            grouped_features = grouping_operation(features, xyz_batch_cnt, idx, roi_batch_cnt)  # (M1 + M2, C, nsample)
            if self.use_xyz:
                new_features = torch.cat([grouped_xyz, grouped_features], dim=1)  # (M1 + M2 ..., C + 3, nsample)
            else:
                new_features = grouped_features
        ...
        new_features[empty_ball_mask] = 0
