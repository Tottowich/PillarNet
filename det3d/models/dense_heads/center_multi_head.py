...
from det3d.models.losses.centernet_loss import FastFocalLoss, RegLoss, IouLoss
...
class CenterMultiHead(nn.Module):
        ...
        **kwargs
        ...
        for num_cls in num_classes:
            ...
            if not dcn_head:
                ...
                self.tasks.append(
                    SepHead(in_channels, heads, bn=True, init_bias=init_bias, final_kernel=3)
                )
            else:
                self.tasks.append(
                    DCNSepHead(in_channels, num_cls, heads, bn=True, init_bias=init_bias, final_kernel=3)
                )
    ...
    def forward(self, feats, *kwargs):
        ...
        for k, task in enumerate(self.tasks):
            task_feat = feats[self.task_strides[k]]
            ret_dicts.append(task(task_feat))
    ...
    def loss(self, example, preds_dicts, test_cfg, **kwargs):
        ...
        for task_id, preds_dict in enumerate(preds_dicts):
            ...
            if 'iou' in preds_dict.keys():
                # IoU loss using predicted boxes
                batch_dim = torch.exp(preds_dict['dim']).detach(); batch_dim = batch_dim.permute(0, 2, 3, 1).contiguous()
                batch_rot = preds_dict['rot'].detach(); batch_rot = batch_rot.permute(0, 2, 3, 1).contiguous()
                batch_rots = batch_rot[..., 0:1]
                batch_rotc = batch_rot[..., 1:2]
                batch_reg = preds_dict['reg'].detach(); batch_reg = batch_reg.permute(0, 2, 3, 1).contiguous()
                batch_hei = preds_dict['height'].detach(); batch_hei = batch_hei.permute(0, 2, 3, 1).contiguous()
                ...
                iou_loss = self.crit_iou(preds_dict['iou'], example['mask'][task_id], example['ind'][task_id],
                                         batch_box_preds, example['gt_box'][task_id])

                loss = hm_loss + self.weight * loc_loss + iou_loss
                ret.update({'loss': loss, 'hm_loss': hm_loss.detach().cpu(), 'iou_loss': iou_loss.detach().cpu(),
