...
from det3d.models.losses.centernet_loss import FastFocalLoss, RegLoss, IouLoss, CIouRegLoss
...
class CenterCIoUHead(nn.Module):
    def __init__(
        self,
        in_channels=[128,],
        tasks=[],
        ...
        super(CenterCIoUHead, self).__init__()
        ...
        self.crit_iou_reg = CIouRegLoss(type=reg_type)
        ...
        for task_id, preds_dict in enumerate(preds_dicts):
            ...
            if self.dataset in ['waymo', 'nuscenes']:
                if 'vel' in preds_dict:
                    ...
                    target_box = target_box[..., [0, 1, 2, -4, -3, -2, -1]]  # remove vel target
                else:
                    preds_dict['anno_box'] = torch.cat((preds_dict['reg'], preds_dict['height'],
                                                        preds_dict['rot']), dim=1)
                    target_box = target_box[..., [0, 1, 2, -2, -1]]  # remove vel target
            else:
            ...
            # calculate IoU loss
            batch_dim = torch.exp(torch.clamp(preds_dict['dim'].clone(), min=-3, max=3))
            batch_dim = batch_dim.permute(0, 2, 3, 1).contiguous()
            batch_rot = preds_dict['rot'].clone().permute(0, 2, 3, 1).contiguous()
            ...
            batch_dim = batch_dim.reshape(batch, H * W, 2)
            ...
            pred_boxes = torch.cat([xs, ys, batch_hei, batch_dim], dim=-1)
            pred_boxes = pred_boxes.permute(0, 2, 1).contiguous().reshape(batch, -1, H, W)

            gt_boxes = example['gt_box'][task_id]
            gt_radius = torch.linalg.norm(gt_boxes[..., 3:5], dim=-1, keepdim=True)
            gt_boxes = torch.cat([gt_boxes[..., :3], gt_radius, gt_boxes[..., 5:6]], dim=-1)
            # to avoid too large number during training
            pred_boxes = torch.clamp(pred_boxes, min=-200., max=200.)
            iou_reg_loss = self.crit_iou_reg(pred_boxes, example['mask'][task_id], example['ind'][task_id],
                                             gt_boxes)

            # loss = hm_loss + iou_loss + (vel_rot_loss + iou_reg_loss) * self.weight
            loss = hm_loss + (loc_loss + iou_reg_loss) * self.weight

            ret.update({'loss': loss, 'hm_loss': hm_loss.detach().cpu(),
                        # 'iou_loss': iou_loss.detach().cpu(),
                        'loc_loss': loc_loss.detach().cpu(), 'iou_reg_loss': iou_reg_loss.detach().cpu(),
    ...
    def predict(self, example, preds_dicts, test_cfg, **kwargs):
        ...
        for task_id, preds_dict in enumerate(preds_dicts):
            ...
            if 'iou' in preds_dict.keys():
                batch_iou = (preds_dict['iou'].squeeze() + 1) * 0.5
            else:
                batch_iou = torch.ones((batch_hm.shape[0], batch_hm.shape[1], batch_hm.shape[2]),
                                        dtype=torch.float32).to(batch_hm.device)
    ...
    def post_processing(self, batch_box_preds, batch_hm, batch_iou, test_cfg, post_center_range, task_id):
        ...
        for i in range(batch_size):
            ...
            scores = scores[mask] * torch.pow(iou_preds[mask], 1)
            labels = labels[mask]
