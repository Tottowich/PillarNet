...
def bbox3d_overlaps_odiou(qboxes, gboxes):
    '''
    ...
    indicator = torch.gt(gboxes[:, 3], 1e-4) & torch.gt(gboxes[:, 4], 1e-4) & torch.gt(gboxes[:, 5], 1e-4) \
                & torch.gt(qboxes[:, 3], 1e-4) & torch.gt(qboxes[:, 4], 1e-4) & torch.gt(qboxes[:, 5], 1e-4)
    ...
    # odiou_h = (torch.max(gboxes[:, 2] + 0.5 * gboxes[:, 5], qboxes[:, 2] + 0.5 * qboxes[:, 5]) -
    ...
    odious[index_loc[:, 0]] = ious[index_loc[:, 0]] - dp - angle_factor[index_loc[:, 0]]
    return odious
