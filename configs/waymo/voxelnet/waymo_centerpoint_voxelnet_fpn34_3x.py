...
model = dict(
    ...
    backbone=dict(
        type="SpMiddleResNetVoxel", num_input_features=5, ds_factor=8),
    neck=dict(
        type="RPNP",
        interp_cfg=dict(
            conv3=dict(planes=64, kernel=2, voxel_size=[0.1*4, 0.1*4, 0.15*4]),
            conv4=dict(planes=128, kernel=1, voxel_size=[0.1 * 8, 0.1 * 8, 0.15 * 8])
        ),
        pool_cfg=dict(radius=0.1 * 8, bev=0.1 * 8),
        layer_nums=[5, 5],
